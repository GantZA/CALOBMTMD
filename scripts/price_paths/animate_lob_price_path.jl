...
lob_densities, raw_price_paths, sample_price_paths, P⁺s, P⁻s, Ps = slob_model(true, 19822) 

anim = @animate for i = 1:100:15000
    l = @layout [a ; b]
    time_step = round(i * slob_model.Δt, digits=0)
    ...
    plt2 = plot(slob_model.x, lob_densities[:,i,1], legend=false, title="t=$time_step",
    xlab="Price", ylab="LOB Density") ;
    plot!(slob_model.x, x -> 0) ;
    plot(plt1, plt2, layout=l)
end
gif(anim, "$(dirname(@__FILE__))/SLOB.gif", fps=8)